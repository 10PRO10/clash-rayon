...
let gameActive = true;
let enemyTowerHP = 1000, playerTowerHP = 1000;
...
class Particle {
    ...
    update() {
        ...
        this.element.style.cssText = `
            position:absolute; width:6px; height:6px; border-radius:50%;
            background:${this.element.style.backgroundColor};
            left:${this.x}px; top:${this.y}px; opacity:${this.life};
            transform:scale(${this.life}); pointer-events:none; z-index:100;
        `;
...
class Unit {
    constructor(type, isPlayer, customX, customY) {
        ...
        Object.assign(this, {
            ...
            attackSpeed: cfg.attackSpeed, range: cfg.range,
            icon: cfg.icon, size: cfg.size, color: cfg.color,
            x: customX ?? (160 + Math.random() * 80),
            y: customY ?? (isPlayer ? 530 : 70),
            lastAttackTime: 0, state: 'moving'
        });
...
function updateTowerHP(bar, hp) { bar.style.width = Math.max(0, (hp / maxTowerHP) * 100) + '%'; }
...
function spawnEnemyUnit() {
    ...
    if (enemyElixir >= cost) { enemyElixir -= cost; enemyUnits.push(new Unit(t, false)); }
}
...
function initMobileControls() {
    ...
    const updateDrag = (e) => {
        ...
        draggedCard.clone.style.left = (clientX - 42) + 'px';
        draggedCard.clone.style.top = (clientY - 52) + 'px';
    ...
    let scrollStart = 0;
    container.addEventListener('touchstart', (e) => { scrollStart = e.touches[0].clientX - container.scrollLeft; }, { passive: true });
    container.addEventListener('touchmove', (e) => {
        if (!draggedCard) container.scrollLeft = scrollStart - e.touches[0].clientX;
    }, { passive: true });
}
